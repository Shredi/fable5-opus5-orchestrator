...
> LIMIT; when it conflicts with context hygiene, the limit wins.
...
Effort: low=mechanical, medium=routine spec work, high=multi-file
...
tests, briefs, standard review. opus takes HARD work DIRECTLY —
...
"uncertain". fable is the escalation CEILING and spends the chair's
own limit: fable spawns START at `high`, round-UP stops there, and
xhigh/max on fable is only for irreversible work, architecture or the
largest closes (a long deliverable there → playbook's LONG OUTPUT
block). Escalation is one-way. On a decline first strip the
...
it closes `V.`. Effort scales with blast radius: an OPUS verifier
takes `max` for architecture / irreversible / security / the largest
closes, `high` for small, low-risk ones; a FABLE verifier defaults to
`high`, `max` only for irreversible or architecture closes. Prefer
opus — it spares the fable limit; security always on opus. Findings
